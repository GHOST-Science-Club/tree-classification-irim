...
class InferenceTransform(nn.Module):
    ...
    @torch.no_grad()
    def forward(self, x):
        # Accept PIL image (dataset path), numpy array (raw path), or tensor.
        ...
        if torch.is_tensor(x):
            if x.ndim == 3 and x.shape[0] in (1, 3, 4):
                ...
                x_t = image_to_tensor(x.cpu().numpy(), keepdim=True).float()
        else:
...
def main():
    ...
    if local_checkpoint_path.exists():
        ...
        raise FileNotFoundError(f"Checkpoint not found at {local_checkpoint_path}. Set inference.checkpoint_path to a local .ckpt path or set inference.wandb_artifact in config.yaml.")
    ...
    if has_labels:
        ...
        for class_id in sorted(metrics_per_class.keys()):
            class_name = reverse_label_map.get(class_id, str(class_id))
        ...
        if wandb.run is not None:
            ...
            print("W&B run not active. Logged metrics locally only.")
    else:
